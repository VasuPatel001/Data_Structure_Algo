...
class Solution:
    def mincostTickets(self, days: List[int], costs: List[int]) -> int:
        table = [0 for _ in range(len(days))]
        table[0] = min(costs)

        for i in range(len(days)):
            # dp[i] to be filled with the cost of travel on days[0...i]
            # days[i] can be covered by a 1-day/7-day/30-days pass

            # if the day i is covered by a 1-day pass, then we have to pick the optimal solution for all previous day
